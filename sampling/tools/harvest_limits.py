...
#
# This file precomputes scan limits and writes them into a card.
#
...
def render_block(card, entries, names, nbins, provenance, sig_rel_unc):
    ...
    for i, entry in enumerate(entries):
        if entry is None:
            ...
            lines.append('    - null')
            continue
        lines.append(f'    # {names[i]}: {len(entry)} bins, in the "channels" order above')
        lines.append('    -')
...
def write_card(card_text, out_path, block, sig_rel_unc, context=None):
    ...
    header = (
        f'\n'
        f'# Precomputed lower/upper limits of the scan, so a run does not have to spend\n'
        f'# hours finding them. Values are TOTAL yields (central value +/- the scan\n'
        f'# range), one entry per patchset in the "patchsets" order above; null means\n'
        f'# "compute at run time".\n'
        f'#\n'
        f'# They are only valid for the settings in scan_limits_context below. Change one\n'
        f'# of those in your run and the sampler says so and recomputes the limits itself\n'
        f'# - correct, but slow. To make the change permanent, paste the limits it prints\n'
        f'# into the block below and update scan_limits_context to match.'
    )
